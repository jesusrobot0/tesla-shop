...
import { generatePaginationLayout } from "@/utils";
...
export function Pagination({ totalPages }: Props) {
  ...
  // Si la pagina actual es menor a 1 o no es un numero redirecciona al path original
  if (currentPage < 1 || isNaN(page)) redirect(pathname);

  // Genera el layout de la paginación
  const paginationLayout = generatePaginationLayout(currentPage, totalPages);
